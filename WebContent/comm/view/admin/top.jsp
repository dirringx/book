...
<div class="head">
	<c:choose>
		...
		<c:when test="${empty manager.name and not manager.name ne 'unseted' and not empty m_student.studentName and m_student.studentName ne 'unseted'}">
			<span class="teacher">欢迎您，${m_student.studentName}</span>
			<a class="loginout" href="${ctx}/admin/a.action?method=logout">退出</a>
		</c:when>
		<c:otherwise>
